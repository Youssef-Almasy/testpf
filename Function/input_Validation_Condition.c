 ...
 * --------------------------
 * Prototype: int input_Validation_Condition(const char *theInputString, unsigned int index)
 * --------------------------
...
{
	return (!theInputString || (theInputString[index] == '%' && !theInputString[index + 1]));
}
